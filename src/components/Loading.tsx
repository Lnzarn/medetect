...
const Loading: React.FC<LoadingProps> = ({
  ...
  const dotsRef = useRef(Array.from({ length: 4 }, () => new Animated.Value(0.2))).current;
  ...
  useEffect(() => {
    ...
    dotsRef.forEach((dot, i) => {
      Animated.loop(
        Animated.sequence([
          Animated.delay(i * 150),
          Animated.timing(dot, { toValue: 1, duration: 250, useNativeDriver: true }),
          Animated.timing(dot, { toValue: 0.2, duration: 250, useNativeDriver: true }),
          Animated.delay(800),
        ])
      ).start();
    });
